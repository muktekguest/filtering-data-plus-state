...
class App extends Component {
  ...
  filterByShowtimes(length) {
    var newMovies = movies.filter(function(movie) {
      return movie.showtimes.length === length
    });
        ...
        <div className="first">
          <ul>
            { movies.map((m) => {
              return (
                <li key={ m.id }>
                  <button onClick={ () => this.showDescription(m.id) }>
                    { m.title }
                  </button>
                </li>
              );
          ...
          <button onClick={ () => this.filterByShowtimes(4) }>Movies with 4 F</button>
          <button onClick={ () => this.filterByShowtimes(5) }>Movies with 5 F</button>
        </div>
        <div className="first">
          <img src={ this.state.cover } />
          <ul>
            { this.state.functions.map(movie => {
              return (
                <li>{ movie.title }</li>
              );
            }) }
